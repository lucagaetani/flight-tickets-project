...
class MySqlQueryGeneratorTypeScript extends import_query_generator.AbstractQueryGenerator {
  ...
  listTablesQuery(options) {
    return (0, import_join_sql_fragments.joinSQLFragments)([
      ...
      (options == null ? void 0 : options.schema) ? `AND TABLE_SCHEMA = ${this.escape(options.schema)}` : `AND TABLE_SCHEMA NOT IN (${this._getTechnicalSchemaNames().map((schema) => this.escape(schema)).join(", ")})`,
